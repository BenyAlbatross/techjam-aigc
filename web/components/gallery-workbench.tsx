...
function TestPanel() {
  ...
  const [model, setModel] = useState("trace_rx_m_v2_vits16_5ep");
  ...
  return (
    <section className="test-panel">
      ...
      <label className="select-label">Model<select value={model} onChange={(event) => setModel(event.target.value)}>
        <option value="trace_rx_m_v2_vits16_5ep">TRACE-RX-M v2 · ViT-S/16 · 5 epochs</option>
      </select></label>
  ...
  return (
    <main className="app-shell">
      ...
      {filtersOpen && view === "gallery" ? <aside className="filter-drawer" aria-label="Filters">
        ...
        <label>Focus model<select value={model} onChange={(event) => setModel(event.target.value)}>{initialData.models.map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}{initialData.images.some((image) => image.predictions.some((prediction) => prediction.model === item)) ? "" : " · external results only"}</option>)}</select></label>
