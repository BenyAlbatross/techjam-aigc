"""Both models on the calibration split under chains of 1..6 sequential transforms.
...
def main():
    ...
    pg = {}
    real = y == 0
    for gname in ["flux_1_schnell", "sdxl_1_0", "gpt_image_2", "gemini_flash_image"]:
        vals = []
        for k in KS:
        ...
        pg[gname] = vals
        flag = "  <- HELD OUT" if gname == "gemini_flash_image" else ""
    ...
    meta.assign(score=f).to_parquet(OUT / f"chains_{WHICH}.parquet", index=False)
    (OUT / f"chain_results_{WHICH}.json").write_text(json.dumps(res, indent=2, default=str))
    print(f"\nwrote runs/chain_results_{WHICH}.json")
