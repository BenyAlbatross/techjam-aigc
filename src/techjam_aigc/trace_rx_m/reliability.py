...
class ReliabilityTable:
    ...
    Development d-prime statistics are independently shrunk within each class.
    ...
    development_authentic: Mapping[Cell, CellStatistics]
    development_aigc: Mapping[Cell, CellStatistics]
    ...
    def fit(
        ...
        *,
        ...
        development_quality: np.ndarray,
        clean_mask: np.ndarray,
        n_bins: int | Iterable[int] = 4,
        prior_strength: float = 20.0,
        variance_floor: float = 1e-6,
    ) -> ReliabilityTable:
        """Fit S5 using disjoint authentic-null and development populations.

        ``clean_mask`` identifies clean development endpoints and is used only
        ...
        dev_logits = _validated_values(development_logits, "development_logits")
        labels = np.asarray(development_labels).reshape(-1)
        clean = np.asarray(clean_mask, dtype=bool).reshape(-1)
        if labels.size != dev_logits.size or clean.size != dev_logits.size:
            raise ValueError("development logits, labels, and clean_mask must have equal length")
        if not np.all(np.isin(labels, [0, 1])):
            raise ValueError("development_labels must be binary (0 authentic, 1 AIGC)")
        ...
        dev_cells = binner.transform(development_quality)
        if null_cells.shape[0] != null_logits.size or dev_cells.shape[0] != dev_logits.size:
        ...
        authentic_stats, _ = fit_shrunk_cell_statistics(
            dev_logits[authentic_mask],
            dev_cells[authentic_mask],
            all_cells,
            prior_strength=prior_strength,
            variance_floor=variance_floor,
        )
        aigc_stats, _ = fit_shrunk_cell_statistics(
            dev_logits[aigc_mask],
            dev_cells[aigc_mask],
            all_cells,
            prior_strength=prior_strength,
            variance_floor=variance_floor,
        ...
        clean_real = dev_logits[clean & authentic_mask]
        clean_aigc = dev_logits[clean & aigc_mask]
        ...
        return cls(
            ...
            development_authentic=authentic_stats,
            development_aigc=aigc_stats,
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "development_authentic": cells(self.development_authentic),
            "development_aigc": cells(self.development_aigc),
    ...
    def from_dict(cls, values: dict[str, object]) -> ReliabilityTable:
        ...
        return cls(
            ...
            development_authentic=statistics("development_authentic"),
            development_aigc=statistics("development_aigc"),
            d_prime_by_cell={
