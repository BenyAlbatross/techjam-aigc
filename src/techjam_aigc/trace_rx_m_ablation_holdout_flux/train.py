...
from techjam_aigc.trace_rx_m_ablation_holdout_flux.config import TraceRXMConfig
from techjam_aigc.trace_rx_m_ablation_holdout_flux.data import (
    BalancedTraceBatchSampler,
    TraceRXMDataset,
    load_training_manifest,
)
from techjam_aigc.trace_rx_m_ablation_holdout_flux.quality import QUALITY_DIMENSION
from techjam_aigc.trace_rx_m_ablation_holdout_flux.reliability import (
...
def _loader(frame: pd.DataFrame, repo_root: Path, config: TraceRXMConfig, *, augment: bool):
    ...
    if augment:
        from techjam_aigc.trace_rx_m_ablation_holdout_flux.augment import SymmetricTransformSampler
...
def run_protocol(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.protocol import run_nuisance_probes
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.training import file_sha256
...
def run_cache(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.training import cache_patch_features, file_sha256
...
def run_capacity(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.integrations import WandbTracker
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.training import (
...
def run_detection(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.integrations import (
        HubCheckpointPublisher,
        WandbTracker,
    )
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.model import TraceRXM
    from techjam_aigc.trace_rx_m_ablation_holdout_flux.training import (
