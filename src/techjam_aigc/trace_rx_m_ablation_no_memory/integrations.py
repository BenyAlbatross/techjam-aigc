...
class HubCheckpointPublisher:
    ...
    def upload_variant_bundle(
        ...
        *,
        ...
        """Preserve the terminal weights for every attempted S4 variant."""
        from huggingface_hub import CommitOperationAdd

        destination = PurePosixPath(self.run_prefix) / "checkpoints" / variant
        result = self.api.create_commit(
            ...
            operations=[
                CommitOperationAdd(
                    path_in_repo=str(destination / "best_detector.pt"),
                    path_or_fileobj=best_path,
                ),
                CommitOperationAdd(
                    path_in_repo=str(destination / "final_detector.pt"),
                    path_or_fileobj=final_path,
                ),
            ],
            commit_message=f"Upload TRACE-RX-M {variant} best and final weights",
        )
        return str(getattr(result, "commit_url", result))
    ...
    def upload_final_bundle(
        ...
        *,
        ...
        weight_paths = [
            ("best_detector.pt", best_path),
            ("final_detector.pt", final_path),
            ("s4_best_detector.pt", best_path),
            ("s4_final_detector.pt", final_path),
            ("s4_detector.pt", shipping_path),
        ]
        if memory_path is not None:
            weight_paths.append(("s3_memory.pt", memory_path))
        operations = [
            CommitOperationAdd(
                path_in_repo=str(destination / name),
                path_or_fileobj=path,
            )
            for destination in (prefix, run_prefix)
