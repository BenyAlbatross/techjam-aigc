...
from techjam_aigc.trace_rx_m_ablation_no_memory.config import TraceRXMConfig
from techjam_aigc.trace_rx_m_ablation_no_memory.data import (
    BalancedTraceBatchSampler,
    TraceRXMDataset,
    load_training_manifest,
)
from techjam_aigc.trace_rx_m_ablation_no_memory.quality import QUALITY_DIMENSION
from techjam_aigc.trace_rx_m_ablation_no_memory.reliability import (
...
def _loader(frame: pd.DataFrame, repo_root: Path, config: TraceRXMConfig, *, augment: bool):
    ...
    if augment:
        from techjam_aigc.trace_rx_m_ablation_no_memory.augment import SymmetricTransformSampler
...
def run_protocol(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    from techjam_aigc.trace_rx_m_ablation_no_memory.protocol import run_nuisance_probes
    from techjam_aigc.trace_rx_m_ablation_no_memory.training import file_sha256
...
def run_cache(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_no_memory.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_no_memory.training import cache_patch_features, file_sha256
...
def run_capacity(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_no_memory.integrations import WandbTracker
    from techjam_aigc.trace_rx_m_ablation_no_memory.training import (
...
def run_detection(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_no_memory.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_no_memory.integrations import (
        HubCheckpointPublisher,
        WandbTracker,
    )
    from techjam_aigc.trace_rx_m_ablation_no_memory.model import TraceRXM
    from techjam_aigc.trace_rx_m_ablation_no_memory.training import file_sha256, seed_everything
    ...
    model = TraceRXM(encoder, config.head)
    ...
    try:
        ...
        tracker.log_model_artifact(
            (best_path, final_path, *metadata_paths),
            metadata={
                "selected_variant": selected_variant,
