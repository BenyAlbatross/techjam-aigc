...
def train_detection_epoch(
    ...
    *,
    ...
    if hasattr(model, "memory") and model.memory.prototypes.requires_grad:
...
def load_detector_checkpoint(
    ...
    *,
    ...
    """Reconstruct the direct-probe detector without an S3 memory artifact."""
    ...
    if artifact.get("source_memory_sha256") != "none":
        raise ValueError("No-memory checkpoint unexpectedly references an S3 artifact.")
    ...
    model = TraceRXM(DinoV3PatchEncoder(backbone_config), config.head)
