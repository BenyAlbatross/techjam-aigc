...
from techjam_aigc.trace_rx_m_ablation_bce_only.config import TraceRXMConfig
from techjam_aigc.trace_rx_m_ablation_bce_only.data import (
    BalancedTraceBatchSampler,
    TraceRXMDataset,
    load_training_manifest,
)
from techjam_aigc.trace_rx_m_ablation_bce_only.quality import QUALITY_DIMENSION
from techjam_aigc.trace_rx_m_ablation_bce_only.reliability import (
...
def _loader(frame: pd.DataFrame, repo_root: Path, config: TraceRXMConfig, *, augment: bool):
    ...
    if augment:
        from techjam_aigc.trace_rx_m_ablation_bce_only.augment import SymmetricTransformSampler
...
def run_protocol(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    from techjam_aigc.trace_rx_m_ablation_bce_only.protocol import run_nuisance_probes
    from techjam_aigc.trace_rx_m_ablation_bce_only.training import file_sha256
...
def run_cache(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_bce_only.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_bce_only.training import cache_patch_features, file_sha256
...
def run_capacity(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_bce_only.integrations import WandbTracker
    from techjam_aigc.trace_rx_m_ablation_bce_only.training import (
...
def run_detection(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_bce_only.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_bce_only.integrations import (
        HubCheckpointPublisher,
        WandbTracker,
    )
    from techjam_aigc.trace_rx_m_ablation_bce_only.model import TraceRXM
    from techjam_aigc.trace_rx_m_ablation_bce_only.training import (
