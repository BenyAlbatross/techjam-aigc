...
Dataset partitions are always ``train``, ``val``, or ``test``. TRACE-RX-M's
train-only memory and detector allocations live in a separate
``training_pool`` field so model-stage bookkeeping cannot be mistaken for a
dataset split. Validation precedes filtering so callers cannot hide leakage.
"""
...
AUTHENTIC_ONLY_POOLS = {"memory", "capacity", "authentic_null"}
...
def load_training_manifest(
    ...
    *,
    ...
    training_pools: Sequence[str] | None = None,
) -> pd.DataFrame:
    ...
    performed before optional split and training-pool selection.
    """
    ...
    if splits is not None:
        requested_splits = {str(split).strip().casefold() for split in splits}
        invalid_splits = requested_splits - DATASET_SPLITS
        if invalid_splits:
            raise ValueError(f"Unsupported requested splits: {sorted(invalid_splits)}")
        selected = selected[selected["split"].isin(requested_splits)]
    if training_pools is not None:
        requested_pools = {str(pool).strip().casefold() for pool in training_pools}
        invalid_pools = requested_pools - TRAINING_POOLS
        if invalid_pools:
            raise ValueError(f"Unsupported requested training pools: {sorted(invalid_pools)}")
        selected = selected[selected["training_pool"].isin(requested_pools)]
    return selected.reset_index(drop=True)


def validate_training_manifest(frame: pd.DataFrame) -> pd.DataFrame:
    """Enforce binary scope, split isolation, training pools, and DDA pairing."""
    ...
    for column in (
        "parent_id", "lineage_id", "split", "training_pool", "label", "sample_kind"
    ):
    ...
    result["split"] = result["split"].str.casefold()
    invalid_splits = sorted(set(result["split"]) - DATASET_SPLITS)
    if invalid_splits:
        raise ValueError(f"Unsupported dataset splits: {invalid_splits}")
    result["training_pool"] = result["training_pool"].str.casefold()
    invalid_pools = sorted(set(result["training_pool"]) - TRAINING_POOLS)
    if invalid_pools:
        raise ValueError(f"Unsupported training pools: {invalid_pools}")
    train = result["split"].eq("train")
    invalid_train_pool = train & result["training_pool"].eq("none")
    invalid_eval_pool = ~train & result["training_pool"].ne("none")
    if invalid_train_pool.any() or invalid_eval_pool.any():
        raise ValueError(
            "train rows require an active training_pool; val/test rows require training_pool=none."
        )
    ...
    authentic_only = result["training_pool"].isin(AUTHENTIC_ONLY_POOLS)
    if (authentic_only & result["sample_kind"].ne("authentic")).any():
        raise ValueError(
            "memory, capacity, and authentic_null training pools accept authentic rows only."
        )
    dda_outside_detector = result["sample_kind"].eq("dda") & result["training_pool"].ne(
        "detector"
    )
    if dda_outside_detector.any():
        raise ValueError("DDA samples are permitted only in the train detector pool.")

    _assert_groups_do_not_cross_partitions(result)
    _validate_dda_pairs(result)
    return result.reset_index(drop=True)


def _assert_groups_do_not_cross_partitions(frame: pd.DataFrame) -> None:
    partition = frame["split"].astype(str) + ":" + frame["training_pool"].astype(str)
    for column in ("lineage_id", *OPTIONAL_LEAKAGE_GROUPS):
        ...
        grouped = (
            frame.loc[present]
            .assign(_group=values[present], _partition=partition[present])
            .groupby("_group")["_partition"]
            .nunique()
        )
        offenders = sorted(grouped[grouped > 1].index.astype(str))
        if offenders:
            raise ValueError(f"{column} crosses dataset/train-pool partitions: {offenders}")
...
def _validate_dda_pairs(frame: pd.DataFrame) -> None:
    ...
    for dda_row, source_id in zip(dda.itertuples(), source_ids.astype(str), strict=True):
        ...
        if (
            source["split"] != "train"
            or source["training_pool"] != "detector"
            or source["split"] != dda_row.split
            or source["training_pool"] != dda_row.training_pool
        ):
            raise ValueError("DDA samples and authentic sources must share the train detector pool.")
...
class TraceRXMDataset:
    ...
    def __init__(
        ...
        *,
        ...
        if preprocessing is not None and preprocessing.image_size != image_size:
            raise ValueError("Dataset image_size and preprocessing.image_size disagree.")
        self.epoch = 0
...
class BalancedTraceBatchSampler:
    ...
    def __init__(
        ...
        *,
        ...
        self.frame = validate_training_manifest(frame).reset_index(drop=True)
        if set(self.frame["split"]) != {"train"} or set(self.frame["training_pool"]) != {
            "detector"
        }:
            raise ValueError("BalancedTraceBatchSampler accepts only the train detector pool.")
        ...
        for kind in required_kinds:
            if not kinds.eq(kind).any():
                raise ValueError(f"Train detector sampling requires {kind!r} rows.")
