...
from techjam_aigc.trace_rx_m_ablation_holdout_gemini.config import TraceRXMConfig
from techjam_aigc.trace_rx_m_ablation_holdout_gemini.data import (
    BalancedTraceBatchSampler,
    TraceRXMDataset,
    load_training_manifest,
)
from techjam_aigc.trace_rx_m_ablation_holdout_gemini.quality import QUALITY_DIMENSION
from techjam_aigc.trace_rx_m_ablation_holdout_gemini.reliability import (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="Run one forward/backward batch without tracking, uploads, or optimizer updates.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume S4 from the latest local periodic checkpoint when present.",
    )
...
def _torch() -> Any:
    ...
    fraction = os.environ.get("TRACE_RX_M_CUDA_MEMORY_FRACTION")
    if fraction and torch.cuda.is_available() and not getattr(_torch, "memory_capped", False):
        value = float(fraction)
        if not 0 < value <= 1:
            raise ValueError("TRACE_RX_M_CUDA_MEMORY_FRACTION must lie in (0, 1].")
        torch.cuda.set_per_process_memory_fraction(value)
        _torch.memory_capped = True
    return torch
...
def _pause_if_reserve_breached(output: Path, *, epoch: int) -> None:
    reserve = float(os.environ.get("TRACE_RX_M_RESERVE_GIB", "0"))
    if reserve <= 0:
        return
    ...
    marker = {
        ...
        "required_reserve_gib": reserve,
        "resume_required": True,
    }
    (output / "memory-pause.json").write_text(json.dumps(marker, indent=2) + "\n")
    raise SystemExit(75)


def _run_preflight(model, loader, *, device, mixed_precision: str) -> None:
    torch = _torch()
    ...
    with autocast:
        loss = model(pixels).logit.float().square().mean()
    loss.backward()
    result = {
...
def _loader(frame: pd.DataFrame, repo_root: Path, config: TraceRXMConfig, *, augment: bool):
    ...
    if augment:
        from techjam_aigc.trace_rx_m_ablation_holdout_gemini.augment import SymmetricTransformSampler
...
def run_protocol(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.protocol import run_nuisance_probes
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.training import file_sha256
...
def run_cache(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.training import cache_patch_features, file_sha256
...
def run_capacity(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.integrations import WandbTracker
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.training import (
...
def _train_detection_variant(
    *,
    ...
    start_epoch = 0
    if resume:
        checkpoints = sorted(checkpoint_directory.glob("epoch-*.pt"))
        if checkpoints:
            artifact = torch.load(checkpoints[-1], map_location="cpu", weights_only=True)
            ...
            if incompatible.unexpected_keys or invalid_missing:
                raise ValueError("Resume checkpoint is incompatible with this ablation model.")
            optimizer.load_state_dict(artifact["optimizer_state"])
            scheduler.load_state_dict(artifact["scheduler_state"])
            history = [EpochMetrics(**item) for item in artifact.get("history", [])]
            start_epoch = int(artifact.get("epoch") or len(history))
            if history:
                best_total = min(item.total for item in history)
                best_epoch = min(range(len(history)), key=lambda i: history[i].total) + 1
    for epoch_index in range(start_epoch, config.optimizer.detection_epochs):
        ...
        if epoch % config.hub.checkpoint_every_epochs == 0:
            ...
            _pause_if_reserve_breached(output, epoch=epoch)
...
def run_detection(args: argparse.Namespace, config: TraceRXMConfig) -> None:
    torch = _torch()
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.backbone import DinoV3PatchEncoder
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.integrations import (
        HubCheckpointPublisher,
        WandbTracker,
    )
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.model import TraceRXM
    from techjam_aigc.trace_rx_m_ablation_holdout_gemini.training import (
    ...
    if heldout_in_train.sum() == 0:
        raise ValueError(
            "The configured held-out generator family is absent from the train detector pool."
        )
    ...
    frozen_encoder = config.backbone.lora_rank == 0
    model.configure_for_detection(frozen_encoder_fallback=frozen_encoder)
    loader, dataset, sampler = _loader(train, args.repo_root, config, augment=True)
    validation_loader, _, _ = _loader(validation, args.repo_root, config, augment=False)
    ...
    try:
        ...
        selected_variant = "frozen" if frozen_encoder else "lora"
        ...
        tracker.log(
            {f"{selected_variant}/held_out_generator/roc_auc": auc},
            step=config.optimizer.detection_epochs,
        )
        gate_passed = auc >= config.data.held_out_min_roc_auc
        ...
        validity = {
            ...
            "roc_auc": auc,
            "variant_roc_auc": {
                selected_variant: auc,
            },
            "threshold": config.data.held_out_min_roc_auc,
            "gate_passed": gate_passed,
            "gate_action": "report_only",
            "frozen_encoder_fallback": False,
        ...
        provenance_path = args.config.parent / "source-provenance.json"
        metadata_paths = tuple(
            path for path in (args.config, validity_path, provenance_path) if path.exists()
        )
        ...
        tracker.summarize({
            ...
            "held_out_generator/gate_passed": gate_passed,
            "frozen_encoder_fallback": False,
            "selected_variant": selected_variant,
        ...
        tracker.log_model_artifact(
            (best_path, final_path, memory_path, *metadata_paths),
            metadata={
                "selected_variant": selected_variant,
